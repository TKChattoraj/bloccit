...
RSpec.describe VotesController, type: :controller do
  ...
  context "signed in user" do
    ...
    describe "POST up_vote" do
      ...
      it "increase the sum of post votes by one" do
        points = user_post.points
        post :up_vote, format: :js, post_id: user_post.id, value: 1
        expect(user_post.points).to eq (points + 1)
      end

      # it ":back redirects to posts show page" do
      #   request.env["HTTP_REFERER"] = topic_post_path(my_topic, user_post)
      #   post :up_vote, post_id: user_post.id, value: 1
    ...
    describe "POST down_vote" do
      ...
      it "returns http success" do
        post :down_vote, format: :js, post_id: user_post.id, value: -1
        expect(response).to have_http_status(:success)
      end
